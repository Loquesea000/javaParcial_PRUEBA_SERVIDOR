...
<div class="container">
    <h1 class="text-center mb-5 mt-5">Menú Director</h1>

    <!-- Almacenar cedulaDirector en la sesión si está en el request -->
    <c:if test="${not empty param.cedulaEstudiante}">
        <c:set var="cedulaEstudiante" value="${param.cedulaEstudiante}" scope="session" />
    </c:if>

    <!-- Si no hay cedulaDirector en el request, tomarla de la sesión -->
    ...
    <c:choose>
        <c:when test="${not empty cedulaEstudiante}">
            <div class="row">
                <!-- Card de Datos del Director con el nuevo estilo -->
                <div class="col-md-8">
                    <div class="user-info h-100">
                        <h3 class="text-center"> Datos del Director</h3>
                        <sql:query dataSource="${DataSource}" var="directorInfo">
                            SELECT * FROM docentes WHERE cedula = ?
                            <sql:param value="${cedulaEstudiante}" />
                        </sql:query>
                        <c:forEach var="director" items="${directorInfo.rows}">
                           <div class="user-image mb-3" style="background-image: url('img/icono_info.png');"></div>

                            <p><i class="fas fa-id-card"></i> Cédula: <strong>${director.cedula}</strong></p>
                            <p><i class="fas fa-user"></i> Nombre: <strong>${director.nombre}</strong></p>
                            <p><i class="fas fa-envelope"></i> Correo: <strong>${director.correo}</strong></p>
                        </c:forEach>
                    </div>
                </div>
            ...
            <!-- Muestra la tabla de anteproyectos donde el director es el responsable -->
    ...
    <thead class="thead-dark">
        <tr>
            ...
            <th>ID Evaluador</th> <!-- Nueva columna para ID Evaluador -->
            <th>Estado</th>
            <th>URL</th>
            <th>Acciones</th>
        </tr>
    ...
    <tbody>
        <c:forEach var="anteproyecto" items="${anteproyectos.rows}">
            <tr>
                ...
                <td>${anteproyecto.id_evaluador}</td> <!-- Mostrar ID Evaluador -->
                <td>
                    <c:choose>
                        <c:when test="${anteproyecto.est == 1 || anteproyecto.est == 3}">
                            <span class="status-pending text-warning">Pendiente</span>
                        </c:when>
                        <c:when test="${anteproyecto.est == 0}">
                            <span class="status-approved text-success">Aprobado</span>
                        </c:when>
                        <c:when test="${anteproyecto.est == 2}">
                            <span class="status-rejected text-danger">Rechazado</span>
                        </c:when>
                        <c:otherwise>
                ...
                <td class="action-buttons">
                    <c:choose>
                        <c:when test="${anteproyecto.est == 1}">
                            <a href="aprobarAnteproyecto.jsp?idAnteproyecto=${anteproyecto.id_anteproyecto}&cedulaDirector=${cedulaDirector}" class="btn btn-success btn-sm">
                                <i class="fas fa-check"></i> Aprobar
                            </a>
                        </c:when>
                    </c:choose>
                    <a href="reprobarAnteproyecto.jsp?idAnteproyecto=${anteproyecto.id_anteproyecto}&cedulaDirector=${cedulaDirector}" class="btn btn-danger btn-sm">
